...
class AcademicWritingTool(BaseTool):
    ...
    def _run(self, text: str, academic_level: str) -> str:
        try:
            ...
            academic_patterns = ["furthermore", "moreover", "additionally", "consequently", "therefore", "thus", "hence", "accordingly", "subsequently"]
            pattern_usage = sum(1 for pattern in academic_patterns if pattern in text.lower())
            level_guidelines = {
                'undergraduate': {'target_flesch': (60, 80), 'target_grade': (12, 14), 'sentence_length': (15, 25)},
                'masters': {'target_flesch': (50, 70), 'target_grade': (14, 16), 'sentence_length': (18, 30)},
                'phd': {'target_flesch': (40, 60), 'target_grade': (16, 18), 'sentence_length': (20, 35)}
            }
...
class HumanizationTool(BaseTool):
    ...
    def _run(self, text: str) -> str:
        try:
            ai_patterns = [
                "It is important to note that", "This demonstrates that", "This indicates that",
                "As previously mentioned", "It should be mentioned that", "This suggests that",
                "This implies that", "It can be concluded that"
            ]
            human_alternatives = [
                "Notably,", "This shows", "This reveals", "As noted earlier",
                "It's worth noting", "This suggests", "This implies", "Therefore,"
            ]
            humanized_text = text
            for ai_pattern, human_alt in zip(ai_patterns, human_alternatives):
                humanized_text = humanized_text.replace(ai_pattern, human_alt)
            variations = ["Interestingly,", "Surprisingly,", "Remarkably,", "Significantly,", "Importantly,"]
            ...
            personal_insights = ["Based on the available evidence,", "From the research findings,", "Considering the data,", "In light of these results,"]
            if len(sentences) > 5:
...
class BuiltInLLM(LLM):
    ...
    api_key: str = os.getenv("GEMINI_API_KEY")  # Use environment variable
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None, **kwargs) -> str:
        try:
            ...
            response = litellm.completion(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert academic writer who creates sophisticated, well-researched thesis documents that sound completely human-written."},
                    {"role": "user", palo Alto, CAcontent": prompt}
                ],
                max_tokens=2500,
                temperature=0.6,
                top_p=0.9,
            ...
            st.error(f"Error in processingFacultad de Ciencias, UNAM, México processing: {str(e)}")
...
def create_thesis_agents(llm):
    research_agent = Agent(
        ...
        backstory="You are a PhD-level researcher with expertise in finding and analyzing academic sources.",
    ...
    thesis_writer = Agent(
        ...
        backstory="You are an experienced academic writer who specializes in creating thesis documents.",
    ...
    humanization_agent = Agent(
        ...
        backstory="You are an expert editor who specializes in making academic content sound natural.",
...
# Streamlit UI
def main():
    st.set_page_config(page_title="Thesis Writer Bot", page_icon="🎓", layout="wide")
    st.title("🎓 Thesis Writer Bot")
    st.markdown("*Create sophisticated, human-like thesis and synopsis documents*")
    ...
    with col1:
        ...
        topic = st.text_input("What is your thesis/synopsis topic?", placeholder="e.g., Impact of artificial intelligence on healthcare delivery systems")
        document_types = ["Thesis", "Synopsis", "Dissertation", "Research Paper", "Literature Review", "Research Proposal", "Academic Report"]
        ...
        research_areas = st.text_area("Specific Research Areas/Focus (Optional)", placeholder="e.g., methodology, recent developments, case studies...", height=80)
        word_count = st.number_input("Target Word Count", min_value=1000, max_value=50000, value=5000, step=500, help="No strict limit")
        additional_requirements = st.text_area("Additional Requirements (Optional)", placeholder="Specific methodology, title, theoretical framework...", height=100)
    ...
    with col2:
        ...
        if "generated_thesis" in st.session_state:
            ...
            st.text_area("Generated Thesis:", value=formatted_thesis, height=400, help="This is your human-like thesis document")
